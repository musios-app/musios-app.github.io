	...
	createCarousel() {
		...
		const slidesHtml = this.sources
			.map((src, i) => {
				...
				const imgClasses = this.settings.constrain ? 'mw-100 mh-100 h-auto w-auto m-auto p-2 top-0 end-0 bottom-0 start-0' : 'h-100 w-100';
				...
				<div class="carousel-item ${!i ? 'active' : ''}" style="min-height: 100px">
					${spinner}
					<div class="ratio ratio-16x9" style="background-color: rgba(255,255,255,0.99); border: solid 2px grey">${inner}</div>
					${caption}
				</div>`;
			})
			.join('');
